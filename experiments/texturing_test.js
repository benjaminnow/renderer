...
// fill in triangle with texture pixel-by-pixel
// texture is 460x460
...
// imagining that the texture covers the full canvas
function get_tex_coord(screen_x, screen_y) {
    let tex_x = screen_x / width * texture.width;
    let tex_y = screen_y / height * texture.height;
    return new utils.Vec2d(tex_x, tex_y);
}

function get_texture_color(x, y) {
    let offset = Math.floor(4 * (y * texture.width + x));
    return [texture.data[offset], texture.data[offset+1], texture.data[offset+2], texture.data[offset+3]];
}

window.preload = function() {
    // loading the texture in as an array
...
window.setup = function() {
    ...
    tri_tex = [get_tex_coord(tri[0].u, tri[0].v), get_tex_coord(tri[1].u, tri[1].v), get_tex_coord(tri[2].u, tri[2].v)]
    pd = pixelDensity();
}
...
function rasterize() {
    ...
    if (p2.v - p1.v > 0) {
        ...
        let tex_start_step = (t2.u - t1.u) / (p2.v - p1.v);
        let tex_end_step = (t3.u - t1.u) / (p3.v - p1.v);
        if (start_step > end_step) {
            let temp = start_step;
            start_step = end_step;
            end_step = temp;
            // same logic for swapping steps for tex, check for tri coords should work
            let temp_tex = tex_start_step;
            tex_start_step = tex_end_step;
            tex_end_step  = temp_tex;
        }
        ...
        for (let y = p1.v; y <= p2.v; y++) {
            ...
            let start_tex_x = t1.u;
            let end_tex_x = t1.u;

            let t_step = 1.0 / (end - start);
            let t = 0.0;
            for (let x = Math.floor(start); x <= Math.floor(end); x++) {
                let tex_x = (1.0 - t) * start_tex_x + t * end_tex_x;
                for (let i = 0; i < pd; i++) {
                    for (let j = 0; j < pd; j++) {
